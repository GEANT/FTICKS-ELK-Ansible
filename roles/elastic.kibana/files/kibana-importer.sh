...
def wait_for_green_status(kibana_base_url, kibana_user, kibana_pass):
  s = requests.session() # HTTP session for keep-alive

  while True:
    try:
      r = s.get(kibana_base_url + '/api/status',auth=( kibana_user, kibana_pass))
      r.raise_for_status()
...
async def upload_kibana_saved_json(kibana_base_url, kibana_user, kibana_pass, jsonArray):
  ...
  for obj in jsonArray:
    typ = obj['type']
    if typ not in ['dashboard', 'search', 'visualization', 'index-pattern']:
      print('Ignoring unknown Kibana export type: {}'.format(typ), file=sys.stderr)
      continue
    else:
      # All `type` cases we handle above happen to have _id and _source fields.
      id = obj['id']
...
#        kibana_base_url + '/api/saved_objects/{}/{}?overwrite=true'.format(typ, id),
        kibana_base_url + '/api/saved_objects/_import',
        json=data, headers=headers, stream=False, auth=(kibana_user, kibana_pass))
...
def main():
  ...
  parser.add_argument('--kibana-user', metavar='kibana', type=str, default='kibana', help='Kibana username (default kibana)')
  parser.add_argument('--kibana-pass', metavar='kibana', type=str, default='kibana', help='Kibana password (default kibana)')
  ...
  if args.wait:
    wait_for_green_status(args.kibana_url, args.kibana_user, args.kibana_pass)

  asyncio.get_event_loop().run_until_complete(upload_kibana_saved_json(args.kibana_url, args.kibana_user, args.kibana_pass, jsonArray))
